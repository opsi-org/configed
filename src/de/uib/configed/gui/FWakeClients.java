...
public class FWakeClients extends FShowList
{
	...
	{
		super(master, title, false, new Object[]{ configed.getResourceValue("FWakeClients.cancel") });
		...
		while (turn < maxSize && !cancelled)
		{
			...
			for (String depot : hostSeparationByDepots.keySet())
			{
				...
				
				{
					...
					//logging.info(this, "act , hostSeparationByDepots.get(depot) "  + hostSeparationByDepots.get(depot));
					//logging.info(this, "act , counterByDepots.get(depot) "  + counterByDepots.get(depot));
					...
					if (executionerForDepots.get(depot) != Executioner.NONE)
					{
						...
						//appendLine(String.format("trying to start up  %4s   %s   from depot   %s ", turn, host, depot));

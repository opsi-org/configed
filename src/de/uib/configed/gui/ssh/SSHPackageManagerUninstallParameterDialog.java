...
public class SSHPackageManagerUninstallParameterDialog extends /*javax.swing.JDialog */ SSHPackageManagerParameterDialog
{
	...
	protected String produceDepotParameter()
	{
		...
		else
		{
			...
			else if (selectedDepots.contains(persist.DEPOT_SELECTION_ALL_WHERE_INSTALLED))
			{
				...
				//logging.debug(this, " we have special selection in possibleDepots " + possibleDepots);
	...
	protected Vector<String> getPossibleDepots()
	{
		...
		for (String depot  : persist.getHostInfoCollections().getDepotNamesList())
		{
			...
			{
				// logging.info(this, " depot "  + depot + " has products " + persist.getDepot2LocalbootProducts().get(depot).keySet()); 
				
				if ( 
					(
						persist.getDepot2LocalbootProducts().get(depot) != null &&
	...
	protected void init()
	{
...

		{
			...
			*/
			...
			buttonUpdateList.setToolTipText(configed.getResourceValue("SSHConnection.ParameterDialog.opsipackagemanager_uninstall.JButtonUpdateList.tooltip"));
	...
	{
		if (tf_selecteddepots.getText().equals(configed.getResourceValue("SSHConnection.command.opsipackagemanager.DEPOT_SELECTION_NODEPOTS")))
			commandPMUninstall.setDepot(null);
		else if (tf_selecteddepots.getText().equals(configed.getResourceValue("SSHConnection.command.opsipackagemanager.DEPOT_SELECTION_ALL_WHERE_INSTALLED")))
			commandPMUninstall.setDepot("all");
		else commandPMUninstall.setDepot(tf_selecteddepots.getText());
	...
	private void initLayout()
	{
		...
		uninstallPanelLayout.setHorizontalGroup(uninstallPanelLayout.createParallelGroup(GroupLayout.Alignment.LEADING)
			...
			.addGroup(uninstallPanelLayout.createSequentialGroup()
					  .addGroup(uninstallPanelLayout.createParallelGroup()
								...
								.addGroup( uninstallPanelLayout.createSequentialGroup()
										   .addGap(5, 10, 20)
										   .addComponent(cb_opsiproducts,Globals.buttonWidth, Globals.buttonWidth, 2*Globals.buttonWidth)
										   .addGap(5, 5, 5)
		...
		uninstallPanelLayout.setVerticalGroup(uninstallPanelLayout.createSequentialGroup()
			  ...
			  .addGroup(uninstallPanelLayout.createParallelGroup(GroupLayout.Alignment.CENTER)
						...
						//.addComponent(buttonUpdateList, Globals.lineHeight, Globals.lineHeight, Globals.lineHeight)
			  ...
			  .addGroup(uninstallPanelLayout.createParallelGroup(GroupLayout.Alignment.CENTER)
						...
						.addComponent(tf_selecteddepots, Globals.buttonHeight,Globals.buttonHeight,Globals.buttonHeight)
					   )
			  ...
			  .addGroup(uninstallPanelLayout.createParallelGroup(GroupLayout.Alignment.CENTER)
						...
						.addComponent(checkb_keepFiles,Globals.buttonHeight,Globals.buttonHeight,Globals.buttonHeight)
						// )
					   )
			  .addGap(Globals.gapSize)

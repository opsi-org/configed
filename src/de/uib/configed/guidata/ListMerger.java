...
public class ListMerger extends ArrayList
{
	...
	static{NO_COMMON_VALUE.setHavingNoCommonValue();}
	//static{ NO_COMMON_VALUE.add("NO COMMON VALUE"); }  // setting a String prevents correct recognition
	...
	public ListMerger merge (List listToMergeIn) 
	{
		if (havingCommonValue) //we were yet in the state of a unique value
		{
			if (!equals(listValue, listToMergeIn))
			{
				...
				//logging.debug(this, "merge first list " + listToMergeIn + " to " + listValue + " havingCommonValue " + havingCommonValue);

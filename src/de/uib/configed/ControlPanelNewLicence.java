...
public class ControlPanelNewLicence extends ControlMultiTablePanel
//tab edit licence 
{
	
	PanelNewLicence thePanel;
	...
	public ControlPanelNewLicence(PersistenceController persist, ConfigedMain mainController)
	{
		thePanel = new PanelNewLicence(this); //extending TabClientAdapter
	...
	public void init()
	{
		...
		JMenuItemFormatted menuItemAddKey = new JMenuItemFormatted(configed.getResourceValue("ConfigedMain.Licences.NewLicencekey"));
		...
		JMenuItemFormatted menuItemPickLicencepool = new JMenuItemFormatted(configed.getResourceValue("ConfigedMain.Licences.MenuItemTransferIDFromLicencepoolToLicencekey"));
		menuItemPickLicencepool.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e)
			{
				...
				while (iter.hasNext() && !keyNew)
				{
					...
					if ( 
						update.getSource() == modelLicencepools
						&& update.keyChanged()
						)
						keyNew = true;
				}
		...
		col=thePanel.panelLicencepools.getColumnModel().getColumn(0); 
		JComboBox comboLP0 = new JComboBox();
		comboLP0.setFont(Globals.defaultFontBig);
		//org.jdesktop.swingx.autocomplete.AutoCompleteDecorator.decorate(combo);
		//combo.setRenderer ();
		//col.setCellEditor(new DefaultCellEditor(combo));
		col.setCellEditor(
			new de.uib.utilities.swing.AdaptingCellEditor(
				comboLP0, 
				new de.uib.utilities.ComboBoxModeller(){
					public ComboBoxModel getComboBoxModel(int row, int column){
						return new DefaultComboBoxModel(
							//new String[]{"a","b"});//
							modelLicencepools.getOrderedColumn(0)
							);
					}
				}
			)
		...
		*/
		...
		classNames.add("java.lang.String"); classNames.add("java.lang.String"); 
		classNames.add("java.lang.String"); classNames.add("java.lang.String");  
		MapTableUpdateItemFactory updateItemFactorySoftwarelicences = new MapTableUpdateItemFactory(modelSoftwarelicences, columnNames, classNames, 0); 
		modelSoftwarelicences =
			new GenTableModel(
				updateItemFactorySoftwarelicences,
				...
				*/
				...
				0, 
				(TableModelListener) thePanel.panelSoftwarelicences,  
				updateCollection);
		...
		col.setCellEditor(
			new AdaptingCellEditor(
				...
				new de.uib.utilities.ComboBoxModeller(){
					public ComboBoxModel getComboBoxModel(int row, int column){
						Vector choicesAllHosts = new Vector( new TreeMap( persist.getHostInfoCollections().getPcListForDepots( mainController.getSelectedDepots() ) ).keySet() );
		...
		// --- PopupMenu
		JMenuItemFormatted menuItemAddLicence = new JMenuItemFormatted(configed.getResourceValue("ConfigedMain.Licences.NewSoftwarelicence"));
		...
		JMenuItemFormatted menuItemPickSoftwarelicence = new JMenuItemFormatted(configed.getResourceValue("ConfigedMain.Licences.MenuItemTransferIDFromSoftwarelicenceToLicencekey"));
		menuItemPickSoftwarelicence.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e)
			{
				...
				while (iter.hasNext() && !keyNew)
				{
					TableEditItem update = (TableEditItem) iter.next();
					if ( 
						update.getSource() == modelSoftwarelicences
						&& update.keyChanged()
						)
						keyNew = true;
				}
				...
				String val = (String) modelSoftwarelicences.getValueAt(
						thePanel.panelSoftwarelicences.getSelectedRowInModelTerms(), 0
						);
				//System.out.println(" ------- found value: " + val);
		...
		thePanel.panelSoftwarelicences.setUpdateController(
			new MapItemsUpdateController(
				thePanel.panelSoftwarelicences,
				modelSoftwarelicences,
				new MapBasedUpdater(){
		...
		JMenuItemFormatted menuItemPickLicencecontract = new JMenuItemFormatted(configed.getResourceValue("ConfigedMain.Licences.MenuItemTransferIDFromLicencecontractToSoftwarelicence"));
		menuItemPickLicencecontract.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e)
			{
				...
				while (iter.hasNext() && !keyNew)
				{
					TableEditItem update = (TableEditItem) iter.next();
					if ( 
						update.getSource() == modelLicencecontracts
						&& update.keyChanged()
						)
						keyNew = true;
				}
				...
				String val = (String) modelLicencecontracts.getValueAt(
						thePanel.panelLicencecontracts.getSelectedRowInModelTerms(), 0
						);
				//System.out.println(" ------- found value: " + val);
				if (thePanel.panelSoftwarelicences.getSelectedRow() == -1)
				{

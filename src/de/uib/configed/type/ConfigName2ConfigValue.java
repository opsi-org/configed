...
public class ConfigName2ConfigValue extends RetrievedMap
{
	...
	protected void buildX()
	{
		...
		{
			List list = null;
			//the retrieved object always are lists, we could correct this by observing the config options
			...
			if (! (retrieved.get(key) instanceof java.util.List) )
			{
				...
				//logging.debug(this, "key " + key + ", retrieved.get(key) " + retrieved.get(key)); 
				//logging.debug(this, "retrieved.get(key) has class " + retrieved.get(key).getClass().getName());
			...
			if (configOptions != null)
			{
				...
				//logging.debug(this, "key " + key + " configOption, class : " + configOption + " ," + configOption.get("classname"));

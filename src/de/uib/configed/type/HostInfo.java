...
public class HostInfo 
{
	...
	public static final String installByShutdown_DISPLAY_FIELD_LABEL = "installByShutdown";
	//---
	...
	static{
		...
		ORDERING_DISPLAY_FIELDS.add( installByShutdown_DISPLAY_FIELD_LABEL );
	...
	public HashMap<String, Object> getDisplayRowMap0()
	{
		...
		unordered.put( installByShutdown_DISPLAY_FIELD_LABEL, clientShutdownInstall );
	...
	public void produceFrom(Map<String, Object> pcInfo)
	{
		...
		// encodeStringFromService is just returning the given value but was used for switching an encoding
		clientDescription =  showValue( configed.encodeStringFromService ((String) pcInfo.get(clientDescriptionKEY)) );
		clientInventoryNumber =  showValue( configed.encodeStringFromService ((String) pcInfo.get(clientInventoryNumberKEY)) );
		clientNotes = showValue( configed.encodeStringFromService ( (String) pcInfo.get(clientNotesKEY) ) );
		clientOneTimePassword = showValue( configed.encodeStringFromService ( (String) pcInfo.get(clientOneTimePasswordKEY) ) );
	...
	public void getInfo(Map<String, Object> pcInfo)
	{
		...
		clientDescription =  configed.encodeStringFromService ((String) pcInfo.get(clientDescriptionKEY));
		if (clientDescription == null)
			clientDescription = "";

		clientInventoryNumber =  configed.encodeStringFromService ((String) pcInfo.get(clientInventoryNumberKEY));
		if (clientInventoryNumber == null)
			clientInventoryNumber = "";

		clientNotes = configed.encodeStringFromService ( (String) pcInfo.get(clientNotesKEY) );
		if (clientNotes == null)
			clientNotes = "";

		clientOneTimePassword = configed.encodeStringFromService ( (String) pcInfo.get(clientOneTimePasswordKEY) );
		...
		if (lastSeen != null)
		{
			...
			if (ls.length >= 15)
				lastSeen = ls[1]+ls[2]+ls[3]+ls[4]+'-'+ls[5]+ls[6]+'-'+ls[7]+ls[8]+' '+ls[9]+ls[10]+':'+ls[11]+ls[12]+':'+ls[13]+ls[14];
		}
		else
		{
			lastSeen = "";
		...
		clientUefiBoot = (Boolean) pcInfo.get(PersistenceController.HOST_KEY_UEFI_BOOT);
	...
	*/
	...
	public void showAndSave(JTableSelectionPanel selectionPanel, MainFrame mainFrame, PersistenceController persist,
	                        String client, Map<String, String> sourceOfChanges)
	{
		...
		if (sourceOfChanges.get(clientInventoryNumberKEY) != null)
		{
			...
			persist.getHostInfoCollections().updateLocalHostInfo(client, clientInventoryNumberKEY, clientInventoryNumber);
		}
		...
		if (sourceOfChanges.get(clientOneTimePasswordKEY) != null)
		{
			...
			persist.getHostInfoCollections().updateLocalHostInfo(client, clientOneTimePasswordKEY, clientOneTimePassword);
		}
		...
		// 	// System.out.println("HostInfo showAndSave shutdownInstall " + shutdownInstall);
		// 	// System.out.println("");System.out.println("");System.out.println("");
			

		// 	String product = "opsi-client-agent";
		// 	persist.setCommonProductPropertyValue( new HashSet(Arrays.asList(client)), product, "on_shutdown_install" , Arrays.asList(shutdownInstall) );
	 	//    					// Set<String> clientNames, String productName, String propertyName,   java.util.List<String> values
		...
		// 	persist.getHostInfoCollections().updateLocalHostInfo(client, clientShutdownInstallKEY, boolShutdownInstall);
		// }
	}
